...
serialPort_t *serialUSART1(uint32_t baudRate, portMode_t mode)
{
    ...
    // DMA TX Interrupt
    nvic.NVIC_IRQChannel = 0; //FIXME: DMA1_Channel4_IRQn;
...
{
    serialPort_t *s = 0; //FIXME: &serialPort1;
...
{
    serialPort_t *s = 0; // FIXME: &serialPort2;
    uint16_t SR = s->USARTx->TDR;

    if (SR & USART_FLAG_RXNE) {
        // If we registered a callback, pass crap there
        if (s->callback) {
            s->callback((uint8_t)s->USARTx->RDR);
        } else {
            s->rxBuffer[s->rxBufferHead] = s->USARTx->RDR;
            s->rxBufferHead = (s->rxBufferHead + 1) % s->rxBufferSize;
        }
    }
    if (SR & USART_FLAG_TXE) {
        if (s->txBufferTail != s->txBufferHead) {
            s->USARTx->RDR = s->txBuffer[s->txBufferTail];
